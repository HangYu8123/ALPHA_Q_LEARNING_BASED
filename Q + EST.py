...
def test(model = 1, episodes= 1000, times  = 1):
    ...
    for t in range(1):
        ...
        QEagent = QL.QLAgent(env.action_space,epsilon = 0.2, mini_epsilon = 0.01, decay = 0.999)
        ...
        for epsd in range(episodes):
            ...
            while(1):
                ...
                prob = Qagent.action_prob(state) + np.asarray(QEagent.action_prob(state)) #* weight
                ...
                est = (Eagent.estimation(next_state) - Eagent.estimation(state)  )
                ...
                Qagent.learning(action,reward,state,next_state)
                QEagent.learning(action,est,state,next_state)
